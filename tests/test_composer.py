...
def _make_sts(
    ...
    *,
    initial: bool = False,
    input_actions: list[dict] | None = None,
    ...
    return {
        ...
        "attributes":      [],
        "locations":       locations,
        "inputActions":    _to_dict(input_actions or []),
        "outputActions":   _to_dict(output_actions or []),
        "guards":          _to_dict(guards or []),
...
def _simple_sts(prefix: str, initial: bool = False) -> dict:
    ...
    guards   = [
        {"id": f"G1_{prefix}", "expression": "true"},
        {"id": f"G2_{prefix}", "expression": "true"},
    ]
    switches = [
        {
            "id":         "r_1",
...
class TestComposeStss:
    ...
    def test_duplicate_guards_are_deduplicated(self) -> None:
        ...
        expressions = [g["expression"] for g in result["guards"].values()]
        # 'true' appears twice across the two STSs but only once in composed output
        assert expressions.count("true") == 1

    def test_duplicate_input_actions_are_deduplicated(self) -> None:
        """Two STSs with identical inputAction text+params share one entry."""
        sts1 = _simple_sts("a", initial=True)
        sts2 = _simple_sts("b", initial=False)
        # Both use "do action" with no params -> should collapse to one entry
        result   = compose_stss([sts1, sts2])
        in_texts = [ia["text"] for ia in result["inputActions"].values()]
